...
if ! dt git user get "${USERNAME}" &>/dev/null; then
...
dt git clone dbci-tools -o "${ORG}" -d "${WORKSPACE_BASE}" -u "${USERNAME}" -p "${PASSWORD}"
echo "✅ dbci-tools cloned"
echo ""

echo "2. Cloning etl-framework..."
dt git clone etl-framework -o "${ORG}" -d "${WORKSPACE_BASE}" -u "${USERNAME}" -p "${PASSWORD}"
echo "✅ etl-framework cloned"
echo ""

echo "3. Cloning demo-dw..."
dt git clone demo-dw -o "${ORG}" -d "${WORKSPACE_BASE}" -u "${USERNAME}" -p "${PASSWORD}"
echo "✅ demo-dw cloned"

DEMO_DW_PATH="${WORKSPACE}/demo-dw"
if [ ! -f "${DEMO_DW_PATH}/Jenkinsfile" ]; then
    echo "   Jenkinsfile not found - initializing with template"
    rsync -av --exclude='.git' "${PROJECT_ROOT}/projects/demo-dw/" "${DEMO_DW_PATH}/"
    if [ -d "${DEMO_DW_PATH}/.git" ]; then
        pushd "${DEMO_DW_PATH}" >/dev/null
        ...
        echo "❌ Error: ${DEMO_DW_PATH} is not a valid git repository"
        exit 1
    fi
else
...
dt git clone demo-etl -o "${ORG}" -d "${WORKSPACE_BASE}" -u "${USERNAME}" -p "${PASSWORD}"
echo "✅ demo-etl cloned"

# Check if demo-etl needs initialization
DEMO_ETL_PATH="${WORKSPACE}/demo-etl"
if [ ! -f "${DEMO_ETL_PATH}/pyproject.toml" ]; then
    echo "   pyproject.toml not found - initializing with template"
    rsync -av --exclude='.git' "${PROJECT_ROOT}/projects/demo-etl/" "${DEMO_ETL_PATH}/"
    if [ -d "${DEMO_ETL_PATH}/.git" ]; then
        pushd "${DEMO_ETL_PATH}" >/dev/null
        git config user.name "${USERNAME}"
        git config user.email "${USERNAME}@${ORG}.demo"
        git checkout -b feature-init
        popd >/dev/null
        ...
        echo "❌ Error: ${DEMO_ETL_PATH} is not a valid git repository"
        exit 1
    fi
else
    echo "   Repository already initialized"
