...
The framework follows the `dbci-tools` pattern: call `run.sh` from the framework directory, targeting a specific ETL project instance.
...
**Options:**
- `--project-dir PATH`: Path to ETL project (default: current directory)
- `--framework-path PATH`: Path to framework (default: auto-detected)
...
```bash
cd /path/to/etl-framework
./run.sh test --project-dir ../demo-etl tests/test_assets.py::test_initial_load -v
```
...
**Validate ETL source model against database schema:**
...
```bash
cd /path/to/etl-framework
./run.sh validate --project-dir ../demo-etl --hcl ../demo-dw/target/schema.hcl -v
```
...
**Options:**
- `--project-dir PATH`: Path to ETL project (default: current directory)
- `--hcl-file FILE, --hcl FILE`: Path to HCL schema file (required)
- `-s FILE, --source-model FILE`: Path to source model YAML (default: auto-detect)
...
# Validate ETL model against schema
cd ../etl-framework
...
./run.sh run --project-dir ../demo-etl -d 2024-02-02
...
./run.sh setup --project-dir ../another-etl --warehouse postgres

# Run pipelines for different projects
./run.sh run --project-dir ../demo-etl -d 2024-02-01
./run.sh run --project-dir ../another-etl -d 2024-02-01

# Test each project
./run.sh test --project-dir ../demo-etl
./run.sh test --project-dir ../another-etl
